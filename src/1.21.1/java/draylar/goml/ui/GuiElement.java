...
public final class GuiElement {
    ...
    void click(int slot, int button, ClickType input, NativeGui gui) {
        this.callback.click(slot, ClickAction.from(button, input), input, gui);
    }

    @FunctionalInterface
    public interface ClickCallback {
        void click(int slot, ClickAction action, ClickType input, NativeGui gui);
    }
    ...
    public static final class ClickAction {
        ...
        private static ClickAction from(int button, ClickType input) {
            boolean right = button == 1;
            return new ClickAction(!right, right, input == ClickType.QUICK_MOVE);
        }
    }
}
