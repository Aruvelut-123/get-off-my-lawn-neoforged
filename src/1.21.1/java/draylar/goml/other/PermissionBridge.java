...
public final class PermissionBridge {
    ...
    private static final Map<ResourceLocation, PermissionNode<Boolean>> BOOLEAN_NODES = new LinkedHashMap<>();
    private static final Map<ResourceLocation, PermissionNode<Integer>> INTEGER_NODES = new LinkedHashMap<>();
    ...
    static {
        ...
        levelNode("command/admin", ADMINS);
        levelNode("command/admin/admin_mode", ADMINS);
        levelNode("command/admin/info", ADMINS);
        levelNode("command/admin/world", ADMINS);
        levelNode("command/admin/general", ADMINS);
        levelNode("command/admin/remove", ADMINS);
        levelNode("command/admin/reload", OWNERS);
        levelNode("command/admin/updateallclaims", OWNERS);
        levelNode("modify_others", ADMINS);
        levelNode("teleport", ADMINS);
    ...
    private static void levelNode(String path, int level) {
        var identifier = id(path);
        BOOLEAN_NODES.put(identifier, new PermissionNode<>(
                identifier,
                PermissionTypes.BOOLEAN,
                (player, uuid, contexts) -> player != null && player.hasPermissions(level)
        ));
    }
    ...
    public static boolean checkPermission(Player player, ResourceLocation permission, int level) {
    ...
    public static boolean checkPermission(CommandSourceStack source, ResourceLocation permission, int level) {
    ...
    public static boolean checkPermission(Player player, ResourceLocation permission, boolean defaultValue) {
        if (player instanceof ServerPlayer serverPlayer) {
            var node = BOOLEAN_NODES.get(permission);
            if (node != null) {
                return PermissionAPI.getPermission(serverPlayer, node);
    ...
    public static boolean checkPermission(CommandSourceStack source, ResourceLocation permission, boolean defaultValue) {
        if (source.getPlayer() instanceof ServerPlayer serverPlayer) {
            var node = BOOLEAN_NODES.get(permission);
            if (node != null) {
                return PermissionAPI.getPermission(serverPlayer, node);
    ...
    public static Predicate<CommandSourceStack> require(ResourceLocation permission, int level) {
        return source -> checkPermission(source, permission, level);
    }

    public static Predicate<CommandSourceStack> require(ResourceLocation permission, boolean defaultValue) {
        return source -> checkPermission(source, permission, defaultValue);
    }

    public static OptionalInt checkPermissionInteger(Player player, ResourceLocation permission) {
        if (player instanceof ServerPlayer serverPlayer) {
