    ...
    protected boolean canPlace(BlockPlaceContext context, BlockState state) {
        ...
        if (!ClaimUtils.isInAdminMode(context.getPlayer())) {
            ...
            var allowedCount2 = PermissionBridge.checkPermissionInteger(context.getPlayer(), id("claim_limit/" +
                    context.getLevel().dimension().location().getNamespace() + "/" + context.getLevel().dimension().location().getPath()));
            ...
            if (GetOffMyLawn.CONFIG.isBlacklisted(context.getLevel(), checkBox.toBox())) {
                context.getPlayer().sendSystemMessage(GetOffMyLawn.CONFIG.prefix(Component.translatable("text.goml.cant_place_claim.blacklisted_area", context.getLevel().dimension().location().toString(), context.getClickedPos().toShortString()).withStyle(ChatFormatting.RED)));
                return false;
            }
        }
